...
function toPublicUser(user) {
  ...
  const username = String(user.user_metadata && user.user_metadata.username || "").trim()
    ...
    || (user.phone ? `用户${String(user.phone).slice(-4)}` : "")
    || "user";
  return {
    id: user.id,
    username,
    email: user.email || "",
    phone: user.phone || "",
    emailVerified: Boolean(user.email_confirmed_at || user.email_verified === 1 || user.emailVerified === true),
...
async function updatePlanBookEntry(entryId, updates) {
  const now = new Date().toISOString();
  if (getSupabaseClient()) {
    await mutateSupabaseState((current) => ({
      ...current,
...
async function updatePlanBookTask(entryId, taskId, updates) {
  const now = new Date().toISOString();
  if (getSupabaseClient()) {
    await mutateSupabaseState((current) => ({
      ...current,
  ...
  const payload = await apiFetch(`/api/plan-book/${encodeURIComponent(entryId)}/task/${encodeURIComponent(taskId)}`, {
    method: "PATCH",
    body: updates
  });
...
async function removePlanBookTask(entryId, taskId) {
  if (getSupabaseClient()) {
    await mutateSupabaseState((current) => ({
      ...current,
      planBookEntries: current.planBookEntries.map((entry) => {
