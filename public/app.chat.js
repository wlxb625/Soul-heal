...
function bindEvents() {
  ...
  window.addEventListener("yuge:mbti-react-ready", () => {
    if (app.isAuthenticated?.()) renderMBTI();
  });

  window.addEventListener("yuge:mbti-navigate", async (event) => {
...
function updateAuthModeUI() {
  ...
  if (isLogin) {
    ...
    els.authPassword.placeholder = "请设置密码（8-64 位，含字母和数字）";
  }
...
function updateAuthMethodUI() {
  ...
  const isLogin = authMode === "login";
  const isPhone = isLogin && authMethod === "phone";
  els.authEmailFields.classList.toggle("hidden", isPhone);
  els.authPhoneFields.classList.toggle("hidden", !isPhone);
  els.authLoginExtras.classList.toggle("hidden", !isLogin || isPhone);

  if (isLogin) {
    if (els.authAccountLabel) els.authAccountLabel.textContent = "用户名或邮箱";
    els.authUsername.type = "text";
    els.authUsername.placeholder = "请输入用户名或注册邮箱";
    els.authUsername.setAttribute("autocomplete", "username");
  } else {
    if (els.authAccountLabel) els.authAccountLabel.textContent = "用户名";
    els.authUsername.type = "text";
    els.authUsername.placeholder = "请输入用户名（2-20 个字符）";
    els.authUsername.setAttribute("autocomplete", "username");
  }

  if (els.authPhoneOtpFields) {
    els.authPhoneOtpFields.classList.toggle("hidden", !isPhone || !phoneOtpRequestedFor);
  }
  if (els.authPhoneSetupHint) {
    els.authPhoneSetupHint.classList.toggle("hidden", !isPhone || Boolean(phoneOtpRequestedFor));
  }
  updateAuthSubmitButton(getAuthIdleSubmitText());
  clearAllAuthErrors();
}

function getAuthIdleSubmitText() {
  if (authMode === "register") return "注册并发送验证邮件";
  if (authMethod === "phone") return phoneOtpRequestedFor ? "验证并登录" : "发送验证码";
  return "登录";
}

function resetPhoneOtpState() {
  phoneOtpRequestedFor = "";
  if (els.authPhoneOtp) els.authPhoneOtp.value = "";
  if (els.authPhoneHint) els.authPhoneHint.textContent = "";
}
...
function setAuthLoading(loading, text) {
  ...
  const idleText = getAuthIdleSubmitText();
...
function clearAllAuthErrors() {
  const errorEls = [
    ...
    els.authConfirmError, els.authPhoneError, els.authPhoneOtpError, els.authServerError
  ];
  errorEls.forEach((el) => {
    if (el) {
      el.textContent = "";
...
function setupAuthFieldValidation() {
  const fields = [
    ...
    { el: els.authPhoneOtp, errorEl: els.authPhoneOtpError, validate: validateAuthPhoneOtp, debounce: 350 }
  ];
...
function validateAuthUsername() {
  ...
  if (authMode === "login") {
    ...
    const isUsername = /^[\u4e00-\u9fff_a-zA-Z0-9_]{2,20}$/.test(val);
    if (!isEmail && !isUsername) return { error: "请输入用户名或有效的邮箱地址" };
    return null;
  }
...
function validateAuthPhoneOtp() {
  const val = els.authPhoneOtp ? els.authPhoneOtp.value.replace(/\D/g, "") : "";
  if (!val) return null;
  if (!/^\d{6}$/.test(val)) return { error: "验证码应为 6 位数字" };
  return null;
}
...
async function requestPhoneOtp() {
  ...
  if (!/^1[3-9]\d{9}$/.test(phone)) {
    showAuthFieldError(els.authPhoneError, "请输入有效的 11 位手机号");
    return false;
  }

  clearAllAuthErrors();
  setAuthLoading(true, phoneOtpRequestedFor ? "重新发送中..." : "发送验证码中...");
  try {
    await app.requestPhoneOtp(phone);
    phoneOtpRequestedFor = phone;
    if (els.authPhoneHint) {
      els.authPhoneHint.textContent = `验证码已发送至 +86 ${phone}，请在 10 分钟内完成验证。`;
    }
    updateAuthMethodUI();
    if (els.authPhoneOtp) els.authPhoneOtp.focus();
    return true;
  } catch (error) {
    showAuthServerError(error.message || "验证码发送失败，请稍后重试");
    return false;
  } finally {
...
async function handleAuthSubmit(event) {
  ...
  const phone = els.authPhone ? els.authPhone.value.replace(/\D/g, "") : "";
  const phoneOtp = els.authPhoneOtp ? els.authPhoneOtp.value.replace(/\D/g, "") : "";

  // Validate
  if (isPhone && isLogin) {
    if (!/^1[3-9]\d{9}$/.test(phone)) { showAuthFieldError(els.authPhoneError, "请输入有效的 11 位手机号"); return; }
    if (!phoneOtpRequestedFor || phoneOtpRequestedFor !== phone) {
      await requestPhoneOtp();
      return;
    }
    if (!/^\d{6}$/.test(phoneOtp)) {
      showAuthFieldError(els.authPhoneOtpError, "请输入 6 位短信验证码");
      return;
    }
  } else if (isLogin) {
    if (!username) { showAuthFieldError(els.authUsernameError, "请输入用户名或注册邮箱"); return; }
    const isEmailLogin = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(username);
    const isUsernameLogin = /^[\u4e00-\u9fff_a-zA-Z0-9_]{2,20}$/.test(username);
    if (!isEmailLogin && !isUsernameLogin) {
      showAuthFieldError(els.authUsernameError, "请输入用户名或有效的邮箱地址"); return;
    }
  ...
  setAuthLoading(true, isPhone ? "验证中..." : (isLogin ? "登录中..." : "注册中..."));

  let result;
  try {
    if (isLogin) {
      if (isPhone) {
        result = await app.verifyPhoneOtp(phone, phoneOtp);
      } else {
        const rememberMe = els.authRememberMe ? els.authRememberMe.checked : false;
        result = await app.login(username, password, rememberMe);
      }
    } else {
      result = await app.register(username, email, password);
    }
  } catch (error) {
    if (error.status === 429 || error.status === 423) {
      ...
      else if (msg.includes("邮箱")) showAuthFieldError(isLogin ? els.authUsernameError : els.authEmailError, msg);
  ...
  if (!isLogin && result && result.requiresEmailVerification) {
    authMode = "login";
    updateAuthModeUI();
    if (els.authUsername) els.authUsername.value = result.email || email;
    showAuthSuccessNotice(`验证邮件已发送至 ${result.email || email}。请点击邮件中的链接后，再使用邮箱和密码登录。`);
    setAuthLoading(false);
    return;
  }
...
function showAuthServerError(message) {
  if (els.authServerError) {
    ...
    const exposesAiConfiguration = /AI 模型|deepseek|Netlify Function/i.test(text);
    els.authServerError.textContent = exposesAiConfiguration
      ? "登录服务暂时繁忙，请稍后重试。"
      : (text || "登录暂时不可用，请稍后重试");
    els.authServerError.classList.remove("auth-server-success");
    els.authServerError.classList.remove("hidden");
    setTimeout(() => els.authServerError.classList.add("hidden"), 8000);
  }
}

function showAuthSuccessNotice(message) {
  if (!els.authServerError) return;
  els.authServerError.textContent = message;
  els.authServerError.classList.add("auth-server-success");
  els.authServerError.classList.remove("hidden");
}
...
async function submitForgotPassword() {
  ...
  try {
    const resp = await app.requestPasswordReset(email);
    if (els.forgotStatus) {
...
async function resendVerificationEmail() {
  ...
  try {
    const user = app.getUser();
    await app.resendEmailVerification(user && user.email);
...
function openCoachWorkspace() {
  switchModule("coach");
  window.requestAnimationFrame(() => els.goalInput?.focus());
}
...
function renderMBTI() {
  ...
  const reactNavigator = document.getElementById("reactMbtiNavigator");
  ...
  if (hasManualMbti) {
    ...
    reactNavigator?.classList.add("hidden");
  ...
  const useReactNavigator = Boolean(window.__YugeMbtiNavigatorReady && reactNavigator);
  els.questionDots.classList.toggle("hidden", useReactNavigator);
  reactNavigator?.classList.toggle("hidden", !useReactNavigator);
  ...
  window.dispatchEvent(new CustomEvent("yuge:mbti-state-change", { detail: state }));
...
function initMouseEffects() {
  ...
  // nested labels and icons, otherwise moving across a button's children makes
  // the cursor ring repeatedly enter and leave.
  const interactiveSelector = "button, a, input, textarea, select, .quick-link-card, .task-card, .ring-card, .conversation-item, .nav-link";
  let hoveredInteractive = null;
  document.addEventListener("pointerover", (e) => {
    const interactive = e.target instanceof Element ? e.target.closest(interactiveSelector) : null;
  ...
  document.addEventListener("pointerout", (e) => {
    ...
    const nextInteractive = e.relatedTarget instanceof Element ? e.relatedTarget.closest(interactiveSelector) : null;
    if (nextInteractive === interactive) return;
    hoveredInteractive = nextInteractive;
    ring.classList.toggle("hover-active", Boolean(nextInteractive));
  });
